...
export type AdminPaymentStatus =
  "unpaid" | "waiting_review" | "deposit_paid" | "paid" | "refunded" | "rejected";
...
export type AdminOrder = {
  ...
  payments?: Array<{
    ...
    method: string;
    amount: number;
    proofs?: Array<{ id: string; image_url: string; storage_path: string }>;
  }>;
};
...
export type AdminAnalytics = {
  ...
  topCustomers: Array<{ name: string; orders: number; revenue: number; last_order: string | null }>;
  latestOrders: Array<{
    order_number: string;
    customer: string;
    total_amount: number;
  ...
  coupons: { most_used: string; total_usage: number; discount_given: number };
  reviews: {
    averageRating: number;
    totalReviews: number;
    highest: Array<{ name: string; rating: number }>;
    lowest: Array<{ name: string; rating: number }>;
  };
};
...
const itemCount = (items: { quantity: number }[] | null | undefined) =>
...
function normalizeOrder(order: AdminOrder & { items?: { quantity: number }[] | null }) {
  return {
    ...
    items: (order.items ?? []).map((item) => ({
      ...
      unit_price: numberValue((item as { unit_price?: unknown }).unit_price),
      total_price: numberValue((item as { total_price?: unknown }).total_price),
    })),
...
export async function updateOrderStatus(id: string, status: AdminOrderStatus) {
  const { error } = await supabase.rpc("update_order_status_with_inventory", {
    p_order_id: id,
    p_status: status,
  });
  fail(error);
...
export async function getStoreSettings() {
  const { data, error } = await supabase
    .from("store_settings")
    .select(
      "brand,whatsapp,email,facebook,instagram,tiktok,primary_color,secondary_color,logo_url,logo_storage_path,vodafone_cash_number,instapay_account,cod_deposit_percentage",
    )
    .eq("id", true)
    .maybeSingle();
  fail(error);
...
export async function saveStoreSettings(input: StoreSettings) {
  ...
  if (
    !Number.isFinite(codDepositPercentage) ||
    codDepositPercentage <= 0 ||
    codDepositPercentage > 100
  ) {
...
export async function getAdminDashboard() {
  const [
    ...
    supabase
      .from("orders")
      .select(
        "id,order_number,customer_id,customer_name,customer_email,status,payment_method,payment_status,total_amount,created_at,items:order_items(quantity)",
      )
      .order("created_at", { ascending: false })
      .limit(4),
    supabase
  ...
  [
    ...
    activityProducts.error,
    activityCategories.error,
    activityCustomers.error,
  ...
  const recentActivity = [
    ...(recentOrdersResult.data ?? []).map((order) => ({
      id: `order-${order.id}`,
      title: "Order placed",
      detail: `${order.order_number} · ${money(numberValue(order.total_amount))}`,
      time: timeLabel(order.created_at),
      created_at: order.created_at,
    })),
    ...(activityProducts.data ?? []).map((product) => ({
      id: `product-${product.id}`,
      title: "Product added",
      detail: product.name,
      time: timeLabel(product.created_at),
      created_at: product.created_at,
    })),
    ...(activityCategories.data ?? []).map((category) => ({
      id: `category-${category.id}`,
      title: "Category added",
      detail: category.name,
      time: timeLabel(category.created_at),
      created_at: category.created_at,
    })),
    ...(activityCustomers.data ?? []).map((customer) => ({
      id: `customer-${customer.id}`,
      title: "Customer joined",
      detail: customer.full_name ?? "Customer",
      time: timeLabel(customer.created_at),
  ...
  return {
    ...
    recentOrders: (
      (recentOrdersResult.data ?? []) as Array<
        AdminOrder & { items?: { quantity: number }[] | null }
      >
    ).map(normalizeOrder),
    recentActivity,
    featuredProducts: (featuredProducts.data ?? []).map((product) => ({
      ...product,
      price: numberValue(product.price),
      category: Array.isArray(product.category) ? (product.category[0] ?? null) : product.category,
    })),
    lowStockProducts: (lowStockProducts.data ?? []).filter(
      (product) => Number(product.stock) <= Number(product.low_stock_threshold),
    ),
...
export async function getAdminAnalytics(period: AnalyticsPeriod): Promise<AdminAnalytics> {
  ...
  return {
    ...
    stats: Object.fromEntries(
      Object.entries(value.stats).map(([key, input]) => [key, number(input)]),
    ) as AdminAnalytics["stats"],
    sales: (value.sales ?? []).map((item) => ({ ...item, revenue: number(item.revenue) })),
    topProducts: (value.topProducts ?? []).map((item) => ({
      ...item,
      stock: number(item.stock),
      quantity_sold: number(item.quantity_sold),
      revenue: number(item.revenue),
    })),
    lowStock: (value.lowStock ?? []).map((item) => ({
      ...item,
      stock: number(item.stock),
      low_stock_threshold: number(item.low_stock_threshold),
    })),
    topCustomers: (value.topCustomers ?? []).map((item) => ({
      ...item,
      orders: number(item.orders),
      revenue: number(item.revenue),
    })),
    latestOrders: (value.latestOrders ?? []).map((item) => ({
      ...item,
      total_amount: number(item.total_amount),
    })),
    coupons: {
      ...value.coupons,
      total_usage: number(value.coupons?.total_usage),
      discount_given: number(value.coupons?.discount_given),
    },
    reviews: {
      ...value.reviews,
      averageRating: number(value.reviews?.averageRating),
      totalReviews: number(value.reviews?.totalReviews),
      highest: value.reviews?.highest ?? [],
      lowest: value.reviews?.lowest ?? [],
    },
  };
}
