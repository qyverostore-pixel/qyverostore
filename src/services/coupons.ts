...
export type CouponInput = Omit<
  Coupon,
  "id" | "usage_count" | "used_count" | "created_at" | "first_order_only"
> & { first_order_only?: boolean };
...
const normalize = (coupon: Coupon): Coupon => ({
  ...coupon,
  ...
  used_count: Number(coupon.usage_count),
});
...
export async function getCoupons(): Promise<Coupon[]> {
  ...
  fail(error);
  return (data ?? []).map((coupon) => normalize(coupon as Coupon));
}
export async function createCoupon(input: CouponInput): Promise<Coupon> {
  const { data, error } = await supabase
    ...
    .single();
  fail(error);
  return normalize(data as Coupon);
}
...
export async function validateCoupon(code: string, subtotal: number): Promise<CouponValidation> {
  ...
  fail(error);
  const result = data?.[0];
  if (!result) throw new Error("Unable to validate coupon.");
  return {
...
export async function redeemCouponForOrder(
  ...
  fail(error);
  const result = data?.[0];
  if (!result) throw new Error("Unable to apply coupon.");
  return { discountAmount: Number(result.discount_amount), finalTotal: Number(result.final_total) };
}
