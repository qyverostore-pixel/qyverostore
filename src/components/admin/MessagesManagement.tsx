...
export function MessagesManagement() {
  ...
  const { data: messages = [] } = useQuery({ queryKey: messageKey, queryFn: getMessages });
        ...
        {messages.map((message) => (
          ...
          >
            ...
            <div className="mt-4 flex gap-2">
              {message.status === "new" && (
                <Button size="sm" variant="outline" onClick={() => void markRead(message.id)}>
                  Mark as read
                </Button>
              )}
              <Button size="sm" variant="ghost" onClick={() => void remove(message.id)}>
                Delete
              </Button>
            </div>
          </article>
