...
function readRecentSearches() {
  ...
  try {
    ...
    return Array.isArray(saved)
      ? saved.filter((item): item is string => typeof item === "string").slice(0, recentLimit)
      : [];
  } catch {
    return [];
  }
...
function SearchResult({
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition ${highlighted ? "bg-teal/10 text-foreground ring-1 ring-inset ring-teal/40" : "text-foreground/80 hover:bg-white/[0.05] hover:text-foreground"}`}
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 truncate text-xs text-muted-foreground">
          {product.category?.name ?? "Collection"} · {Number(product.price).toLocaleString()} EGP
        </p>
      </div>
      <div className="shrink-0 text-right text-xs text-muted-foreground">
        <span className="flex items-center justify-end gap-1 text-amber-300">
          <Star className="size-3 fill-current" />
          {Number(product.rating).toFixed(1)}
        </span>
        <span className={`mt-1 block ${product.stock > 0 ? "text-teal" : "text-red-300"}`}>
...
function SearchSkeleton() {
  return (
    <div className="space-y-1" aria-label="Searching products">
      {Array.from({ length: 4 }, (_, index) => (
        <div key={index} className="flex items-center gap-3 rounded-xl px-3 py-3">
          <div className="size-12 animate-pulse rounded-xl bg-white/[0.07]" />
              ...
              <section>
                {isSearching ? (
                  ...
                  <div className="py-10 text-center">
                    <span className="mx-auto grid size-12 place-items-center rounded-2xl border border-white/10 bg-white/[0.035] text-teal">
                      <Search className="size-5" />
                    </span>
                    <p className="mt-4 text-sm text-foreground">No products found</p>
                  </div>
                )}
              </section>
            ) : (
