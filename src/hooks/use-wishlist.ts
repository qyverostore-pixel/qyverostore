...
export function useWishlistToggle(product: StoreProduct) {
  ...
  const mutation = useMutation<
    ...
    Error,
    WishlistMutationVariables,
    WishlistMutationContext
  >({
    mutationFn: async ({ action, product: selectedProduct }) => {
      if (action === "remove")
        return { action: "removed", record: await removeFromWishlist(selectedProduct.id) };
    ...
    onMutate: async ({ action, product: selectedProduct }) => {
      ...
      queryClient.setQueryData<WishlistItem[]>(wishlistKeys.all, (current = []) =>
        action === "remove"
          ? current.filter((item) => item.product_id !== selectedProduct.id)
          : [
              {
                id: `pending-${selectedProduct.id}`,
                customer_id: "",
    ...
    onSuccess: (result) => {
      if (result.action === "removed") {
        queryClient.setQueryData<WishlistItem[]>(wishlistKeys.all, (current = []) =>
          current.filter((item) => item.product_id !== result.record.product_id),
        );
      }
  ...
  const toggle = useCallback(() => {
    ...
    const action: WishlistAction = current.some((item) => item.product_id === product.id)
      ? "remove"
      : "add";
