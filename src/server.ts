...
async function seoResponse(pathname: string) {
  ...
  const staticPages = ["", "/products", "/about", "/contact", "/connect", "/policies", "/privacy-policy", "/terms-and-conditions", "/return-policy", "/shipping-policy"];
  ...
  if (supabaseUrl && supabaseKey) {
    try {
      ...
      const [productResponse, categoryResponse] = await Promise.all([
        fetch(`${supabaseUrl}/rest/v1/products?select=slug,updated_at&is_active=eq.true&status=in.(active,out_of_stock)`, { headers }),
  ...
  const entries = [
    ...
    ...categories.map((category) => `<url><loc>${xml(`${siteUrl}/products?category=${encodeURIComponent(category.slug)}`)}</loc></url>`),
    ...products.map((product) => `<url><loc>${xml(`${siteUrl}/products/${product.slug}`)}</loc>${product.updated_at ? `<lastmod>${new Date(product.updated_at).toISOString()}</lastmod>` : ""}</url>`),
  ].join("");
